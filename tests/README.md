...
`test_serpapi.py` cobre parâmetros da consulta, normalização, tokens, erros, chave ausente e limite de orçamento. `test_monitor.py` cobre seis rotas fixture, limite mensal, quota, falha parcial, preservação de dado antigo, duplicata diária, schema público, lock e alerta local. `test_telegram.py` cobre escape, deduplicação, queda, teto, persistência de falha, confirmação e resposta Telegram.
